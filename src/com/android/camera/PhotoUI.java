...
public class PhotoUI implements PieListener,
    ...
    public PhotoUI(CameraActivity activity, PhotoController controller, View parent) {
		...
		ViewStub surfaceStub = (ViewStub)mRootView.findViewById(R.id.surfaceview_stub);
		if(surfaceStub != null) {
			surfaceStub.inflate();
        ...
        surfaceContainer.addOnLayoutChangeListener(new OnLayoutChangeListener() {
            ...
            public void onLayoutChange(View v, int left, int top, int right,
              ...
              //  tryToCloseSubList();
                 ...
                 mMaxPreviewWidth = 1080;
                mMaxPreviewHeight = 1920;
...
//        });
        ...
        // init default preview ratio , solve the problem to 16:9 or 4:3 charger issue
    ...
    public void setAspectRatio(float ratio) {
		...
		int r = CameraUtil.determineRatio(mAspectRatio);
		if(r != CameraUtil.RATIO_4_3) {
			if(r == CameraUtil.RATIO_1_1) {
				mSwitcher.movePosition( true);
			} else{
				mSwitcher.movePosition(false);
			}
		}  else{
			mSwitcher.movePosition(false);
		}
    ...
    public void layoutPreview(float ratio) {
        ...
        if (mScreenRatio == CameraUtil.RATIO_16_9
			...
			Log.d(TAG,"layoutPreview mOrientationResize:"+mOrientationResize);
            if (mOrientationResize) {
                ...
                if (width > height) {
					...
					Log.d(TAG,"layoutPreview scaledTextureWidth:"+scaledTextureWidth+",scaledTextureHeight:"+scaledTextureHeight);
                }
            }

            Log.v(TAG, "setTransformMatrix: scaledTextureWidth = " + scaledTextureWidth
                    + ", scaledTextureHeight = " + scaledTextureHeight+",rotation:"+rotation);
            ...
            if(mScreenRatio == CameraUtil.RATIO_4_3) {
			 ...
			 if(mScreenRatio == CameraUtil.RATIO_1_1) {
                lp.bottomMargin = 105;
            }
        }
      ...
      if(mSurfaceView.getHeight() != lp.height){
		...
		if(img_surfaceview_cover != null) {
			img_surfaceview_cover.setLayoutParams(lp);
		}
		if(mImageView != null) {
    ...
    public void switchMakeup(){
        this.mSwitcher.makeup = true;
    }

    public boolean isMakeup(){
        return this.mSwitcher.isMakeup();
    }

    public void onCameraOpened(PreferenceGroup prefGroup, ComboPreferences prefs,
            Camera.Parameters params, OnPreferenceChangedListener listener, MakeupLevelListener makeupListener,int mCameraId) {
		...
		showOrHideFlashButton(mCameraId);
        ...
        if (mGestures == null) {
            ...
            mGestures = new PreviewGestures(mActivity, this, mZoomRenderer, mPieRenderer,mSwitcher);
            mRenderOverlay.setGestures(mGestures);
        }
        mGestures.setPhotoMenu(mMenu);
	mGestures.setCurModeIndex(ModuleSwitcher.PHOTO_MODULE_INDEX);
        ...
        mSwitcher.initSeekBar(prefGroup, makeupListener);
    }
...
//        mSwitcher.closePopup();
    ...
    public void setSwitcherIndex() {
        mSwitcher.renderSelection(ModuleSwitcher.PHOTO_MODULE_INDEX);
    }
	...
	public void onMoveUpAndDown(int flag) {
		 mController.onMoveUpAndDown(flag);
	}
...
//            return true;
    ...
    public void showPopup(ListView popup, int level, boolean animate) {
        ...
        // hideUI();
        LinearLayout list_menu_root = (LinearLayout) mActivity.getLayoutInflater().inflate(R.layout.list_menu_root, null);
        list_menu_root.setOnClickListener(new View.OnClickListener(){
            @Override
            public void onClick(View v){}
        });
        //list_menu_root.setMinimumHeight(mRootView.getHeight());
        ImageButton btn_back = (ImageButton) list_menu_root.findViewById(R.id.btn_back);
        btn_back.setOnClickListener(new View.OnClickListener(){
            @Override
            public void onClick(View v){mMenu.handleBackKey();}
        });
        ...
        if (level == 2) {
            ...
            // } else {
                params = new FrameLayout.LayoutParams(
                        ...
                        Gravity.RIGHT | Gravity.TOP);
            // }
            // params.topMargin = mCameraControls.getMenuSize();
            ...
            list_menu_root.addView(popup);
            mSubMenuLayout.addView(list_menu_root);
           // mSubMenuLayout.setOrientation(mOrientation, true);
           // mSubMenuLayout.setBackgroundColor(Color.WHITE);
            mSubMenuLayout.bringToFront();
            TextView title = (TextView) list_menu_root.findViewById(R.id.title);
            title.setText(((ListSubMenu) popup).getTitle());
        }
		mMenu.setSettingPreview(mMenuLayout);
        if (animate) {
            if (level == 1)
                // mMenu.animateSlideIn(mMenuLayout, CameraActivity.SETTING_LIST_WIDTH_1, true);
                mMenu.animateSlideInY(mMenuLayout);
            if (level == 2)
                // mMenu.animateSlideIn(mSubMenuLayout, CameraActivity.SETTING_LIST_WIDTH_1, true);
                mMenu.animateSlideInX(mSubMenuLayout);
    ...
    public void removeLevel2() {
        if (mSubMenuLayout != null) {
            ...
            mSubMenuLayout.setBackground(null);
        }
    }
    ...
    public boolean sendTouchToPreviewMenu(MotionEvent ev) {
        if (mPreviewMenuLayout != null) {
            ev.offsetLocation(0, -(mMenu.getPreviewMenuSize() - 70));
...
//        mSwitcher.closePopup();
    ...
    public void enableBokehFocus(boolean enable) {
        if (mPieRenderer != null && mIsBokehMode) {
            ...
            if (mBlurDegreeProgressBar != null) {
                ...
                Log.d(TAG,"enableBokehFocus enable:"+enable);
                    mBlurDegreeProgressBar.setVisibility(View.GONE);
                }
            }
        }
   ...
   /** public boolean isPreviewCoverVisible() {
    ...
    public void enableFlashSwitcher(boolean enable){
        mMenu.enableFlashSwitcher(enable);
    }

    public void enableHdrSwitcher(boolean enable){
        mMenu.enableHdrSwitcher(enable);
    }

    public void enableFrontBackSwitcher(boolean enable){
        mMenu.enableFrontBackSwitcher(enable);
    }
    ...
    public void setOrientation(int orientation, boolean animation) {
        ...
        if (mPreviewMenuLayout != null) {
            ...
            if(vg != null && vg.getId() == R.id.frame_flash)
                return;
            if (vg != null)
	...
	public void onExposure(int value) {
		 mController.setExposure(value);
	}
	public void showOrHideFlashButton(int cameraId) {
		mMenu.showOrHideFlashButton(cameraId);
	}
