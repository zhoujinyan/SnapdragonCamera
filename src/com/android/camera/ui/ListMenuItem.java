...
public class ListMenuItem extends RelativeLayout {
    ...
    public void initialize(ListPreference preference) {
        ...
        if (preference == null)
            return;
        setIcon(preference);
        if(preference.getEntryValues().length != 2 || CameraSettings.KEY_CAMERA_SAVEPATH.equals(preference.getKey())){
            mSwitch.setVisibility(View.GONE);
        }else{
            mSwitch.setChecked(preference.findIndexOfValue(preference.getValue()) == 1 ? true : false);
            mSwitch.setOnCheckedChangeListener(new OnCheckedChangeListener() {
                @Override
                public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                Log.d(TAG,"onCheckedChanged mIndex:"+mIndex);
                    changeIndex(1 - mIndex);
                }
            });
        }
