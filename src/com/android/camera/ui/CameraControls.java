...
public class CameraControls extends RotatableLayout {
    ...
    private static final int FLASH_INDEX = 3;
    ...
    private void setLocation(int w, int h) {
        int rotation = getUnifiedRotation();
        if (mIsBokehMode) {
            toIndex(mFrontBackSwitcher, w, h, rotation, 5, 6, FRONT_BACK_INDEX);
        ...
        toIndex(mFlashSwitcher, w, h, rotation, 1, 0, FLASH_INDEX);
    ...
    public void setBokehMode(boolean enable) {
		Log.d(TAG,"setBokehMode enable:"+enable);
        mIsBokehMode = enable;
        if (mIsBokehMode) {
            WIDTH_GRID = 6;
        } else {
...
//        mRemainingPhotos.setVisibility(View.INVISIBLE);
    ...
    private void toIndex(View v, int w, int h, int rotation, int index, int index2, int index3) {
		...
		if(windex >0 && hindex == 0) {
		    cx = (2 * windex + 1) * boxw / 2 -100;
			cy = (2 * hindex + 1) * boxh / 2 -100;
			tw += 40;
		} else{
		    cx = (2 * windex + 1) * boxw / 2;
			cy = (2 * hindex + 1) * boxh / 2;
		}
    ...
    private void layoutSwitcher(int screenWidth, int screenHeight) {
//        FrameLayout.LayoutParams lp = (FrameLayout.LayoutParams) mSwitcher.getLayoutParams();

        int width = mSwitcher.getMeasuredWidth();
        int height = mSwitcher.getMeasuredHeight() ;

        int left = 0;
        int right = width;
        int bottom = screenHeight - mBottomMargin+110;
        int top = bottom - height;
        mSwitcher.layout(left, top, right, bottom);
    }
    ...
    public void setOrientation(int orientation, boolean animation) {
        ...
        View[] views = {
            mFlashSwitcher, mSceneModeSwitcher, mFilterModeSwitcher, mFrontBackSwitcher,
            TsMakeupManager.HAS_TS_MAKEUP ? mTsMakeupSwitcher : mHdrSwitcher,mBokehSwitcher,
            mMenu, mShutter, mPreview, mMute, mReviewRetakeButton,
