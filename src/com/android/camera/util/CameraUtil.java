...
public class CameraUtil {
    ...
    private final static int MINIMAL_BATTERY_PENCENTAGE_WITH_CHARGING = 10;
    private final static int MINIMAL_BATTERY_PENCENTAGE_WITHOUT_CHARGING = 15;
    ...
    private static int spaceTime = 350;
	private static float preDegree = -1;
	private static float videoPreDegree = 0;
	private static float photoPreDegree = 0;
	public static boolean isTsMakeUp = true; //true :Snapdragon beauty  false:arcsoft beauty
	public static int thumbCount = 0;
	...
	private static long secondLastClickTime=0;
    private static int secondSpaceTime = 350;
	public static boolean isSecondFastScroll() {
		long currentTime = System.currentTimeMillis();
		boolean isFastScroll = false;
		if (currentTime - secondLastClickTime > secondSpaceTime) {
		  isFastScroll= false;
		   secondLastClickTime = currentTime;
		} else {
		  isFastScroll = true;
		}
		return isFastScroll;
    ...
    private static long threeLastClickTime=0;
    private static int threeSpaceTime = 350;
	public static boolean isFastClick() {
		long currentTime = System.currentTimeMillis();
		boolean isFastClick = false;
		if (currentTime - threeLastClickTime > threeSpaceTime) {
		  isFastClick= false;
		  threeLastClickTime = currentTime;
		} else {
		  isFastClick = true;
		}
		return isFastClick;
	...
	private static long switcherLastClickTime=0;
    private static int switcherSpaceTime = 500;
	public static boolean isFastSwitcher() {
		long currentTime = System.currentTimeMillis();
		boolean isFastSwitcher = false;
		if (currentTime - switcherLastClickTime > switcherSpaceTime) {
		  isFastSwitcher= false;
		  switcherLastClickTime = currentTime;
		} else {
		  isFastSwitcher = true;
		}
		return isFastSwitcher;
	}
	
	private static long leftOrRightLastClickTime=0;
    private static int leftOrRightSpaceTime = 500;
	public static boolean isThreeFastScroll() {
		long currentTime = System.currentTimeMillis();
		boolean isFastScroll = false;
		if (currentTime - leftOrRightLastClickTime > leftOrRightSpaceTime) {
		  isFastScroll= false;
		  leftOrRightLastClickTime = currentTime;
		} else {
		  isFastScroll = true;
		}
		return isFastScroll;
	...
	public static float getPreDegree() {
	    return preDegree;
	}

	public static void setPreDegree(float degree) {
		preDegree = degree;
	}

	public static float getVideoPreDegree() {
	    return videoPreDegree;
	}

	public static void setVideoPreDegree(float degree) {
		videoPreDegree = degree;
	}

	public static float getPhotoPreDegree() {
	    return photoPreDegree;
	}

	public static void setPhotoPreDegree(float degree) {
		photoPreDegree = degree;
	}
    ...
    public static int getCameraFacingIntentExtras(Activity currentActivity) {
        ...
        if (isFrontCameraIntent(intentCameraId)) {
        	...
        	if(intentCameraId>0 && intentCameraId<= 3) {
				cameraId = intentCameraId;
			}
		}
        return cameraId;
    }
   ...
   public static String getFilpModeString(int value){
    ...
    public static int determineRatio(float ratio) {
        ...
        if (ratio == 1.0f) {
            return RATIO_1_1;
        } else if (ratio > 1.33f && ratio < 1.34f) {
            return RATIO_4_3;
	...
	public static boolean isPowerGood(Context context) {
        ...
        int percentage = 0;
        if (scale > 0) {
            percentage = (100 * level) / scale;
        }

        if (isCharging && percentage >= MINIMAL_BATTERY_PENCENTAGE_WITH_CHARGING) {
            return true;
        }

        if (!isCharging && percentage >= MINIMAL_BATTERY_PENCENTAGE_WITHOUT_CHARGING) {
            return true;
        }
