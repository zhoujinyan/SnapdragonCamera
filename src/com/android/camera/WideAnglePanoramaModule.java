...
public class WideAnglePanoramaModule
	...
	//public static final int DEFAULT_CAPTURE_PIXELS = 1280 * 1080;
	...
	private static final int ON_AUTO_FOCUS = 7;
    ...
    public void autoFocus() {
        ...
        mCameraDevice.autoFocus(mMainHandler, mAutoFocusCallback);
    ...
    public void onScreenSizeChanged(int width, int height) {
    	Log.d(TAG,"onScreenSizeChanged width:"+width+",height");
    ...
    private void initializeFocusManager() {
       ...
       private void showTapToFocusToastIfNeeded() {
        ...
        if (mFocusAreaSupported &&
            ...
            mMainHandler.sendEmptyMessageDelayed(SHOW_TAP_TO_FOCUS_TOAST, 1000);
        }
    }
            ...
            switch (fCameraState) {
                ...
                default:
                    ...
                    break;
                        ...
                        if(mMainHandler.hasMessages(ON_AUTO_FOCUS)) {
                               mMainHandler.removeMessages(ON_AUTO_FOCUS);
                        }
                       ...
                       msg.setData(bundle);
                       mMainHandler.sendMessageDelayed(msg, AUTO_FOCUS_DELAY);
        }
    }
    ...
    private void updateAutoFocusMoveCallback() {
    	Log.d(TAG,"updateAutoFocusMoveCallback getFocusMode:"+mParameters.getFocusMode());
        if (mParameters.getFocusMode().equals(CameraUtil.FOCUS_MODE_INFINITY)) {
            mCameraDevice.setAutoFocusMoveCallback(mMainHandler,
    ...
    public void startFaceDetection() {
        ...
        if (mParameters.getMaxNumDetectedFaces() > 0) {
			...
			int cameraId = CameraHolder.instance().getBackCameraId();
            CameraInfo info = CameraHolder.instance().getCameraInfo()[cameraId];
            mUI.onStartFaceDetection(mDeviceOrientation,
                    (info.facing == CameraInfo.CAMERA_FACING_FRONT));
            mCameraDevice.setFaceDetectionCallback(mMainHandler, mUI);
    ...
    private boolean findBestPreviewSize(List<Size> supportedSizes, boolean need4To3,
        ...
        for (Size size : supportedSizes) {
            int h = size.height;
            int w = size.width;
			Log.d(TAG,"findBestPreviewSize width:"+w+",height:"+h);
    ...
    public void startCapture() {
		...
		int mState = mFocusManager.getFocusState();
		 if (mFocusManager != null && (mState == mFocusManager.STATE_FOCUSING ||
                    mState == mFocusManager.STATE_SUCCESS || mState == mFocusManager.STATE_FAIL)) {
   ...
   public boolean isPreviewReady() {
    ...
    public void onShutterButtonClick() {
        ...
        if (mPaused || mThreadRunning || mCameraTexture == null || !isPreviewReady()) {
            return;
        }

        if(mCaptureState == CAPTURE_STATE_MOSAIC && System.currentTimeMillis() - mTimeTaken < 2000) {
            return;
        }
    ...
    private void initMosaicFrameProcessorIfNeeded() {
        ...
        int width = (mCameraPreviewWidth * perct) / 140;
        int height = (mCameraPreviewHeight * perct) / 140;
        if ((0 < width) && (0 < height)) {
    ...
    public void onPauseAfterSuper() {
		...
		if (fCameraState != PREVIEW_STOPPED) {
            mCameraDevice.cancelAutoFocus();
        }
...
//xiayy added
	mUI.setPreviewGesturesPanoUI();
		mUI.enableShutter(true);
		mUI.enableSwitch(true);
        mUI.setSwitcherIndex();
		mUI.showOrHideSun(false);
        keepScreenOnAwhile();
    ...
    public void onSingleTapUp(View view, int x, int y) {
		if (mPaused || mCameraDevice == null || fCameraState == PREVIEW_STOPPED) {
            return;
        }
    ...
    public void runInPreviewFrame(byte[] data, CameraProxy camera) {  
		 Log.d(TAG, "~~~~runInPreviewFrame~~");
			...
			//blurNative();
			/**Bitmap gaussianBlurImg = nativeBlur.blur(bitmap, 70f);
			mActivity.mImageView.setImageBitmap(gaussianBlurImg);
			mActivity.mImageView.setVisibility(View.VISIBLE);*/
	 		/**AlphaAnimation inAnimation = new AlphaAnimation(0f, 1.0f);
            inAnimation.setDuration(250);
     		mActivity.mImageView.startAnimation(inAnimation);*/
