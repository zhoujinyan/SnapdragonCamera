...
import com.android.camera.ui.PhotoViewPager;
...
public class CameraActivity extends Activity
    ...
    private static final boolean ENABLE_RECORD_LOCATION = false;
	...
	private static final String KEY_SECURE_GALLERY = "secure_gallery";
	...
	private static final long EXIT_APPLICATION_TIMEOUT = 1000*60;
    ...
    private FrameLayout camera_above_filmstrip_layout_secure;
    private PhotoViewPager imgs_viewpager;
    private AnimationSet mShowAction, mHideAction;
	...
	private static final String alarmAction = "android.alarm.demo.action";
	private static final String COM_QUCII_SNAPCAM_EXIT = "com.qucii.snapcam.exit";
	private static final String COM_QUCII_SNAPCAM_ENTER = "com.qucii.snapcam.enter";
	private StatusBarManager mStatusBarManager;
	private LockPatternUtils mLockPatternUtils;

	private void setAlarmTime(long triggerAtMillis) {  
            am = (AlarmManager)getSystemService(Context.ALARM_SERVICE);  
            Intent intent = new Intent(alarmAction);  
            sender = PendingIntent.getBroadcast(  
                    this, 0, intent, PendingIntent.FLAG_UPDATE_CURRENT);   
            long time =  SystemClock.elapsedRealtime()+triggerAtMillis;
            am.set(AlarmManager.RTC_WAKEUP, triggerAtMillis, sender);  
			am.setWindow(AlarmManager.ELAPSED_REALTIME_WAKEUP , time, 0, sender);
     }  

	private class AlarmReceiver extends BroadcastReceiver {  
        public void onReceive(Context context, Intent intent) {  
            if (alarmAction.equals(intent.getAction())) {  
			    Log.d(TAG,"handleMessage killProcess:"+android.os.Process.myPid());
	...
	public void unregisterAlarmReceiver() {
	 ...
	 public void startExit(){
        if(isStart == false) {
           ...
           setAlarmTime(EXIT_APPLICATION_TIMEOUT);
        }
    }
   ...
   public void stopExit(){
    ...
    private BroadcastReceiver mBatteryLowReceiver = new BroadcastReceiver(){
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if(mCurrentModule != null) {
                mCurrentModule.onBatteryLow(intent);
            }
        }
    };
    ...
    public void gotoGallery() {
        ...
        try {
			...
			KeyguardUpdateMonitor updateMonitor = KeyguardUpdateMonitor.getInstance(this);
        	boolean canSkipBouncer = updateMonitor.getUserCanSkipBouncer(
                KeyguardUpdateMonitor.getCurrentUser());
       		boolean secure = mLockPatternUtils.isSecure(KeyguardUpdateMonitor.getCurrentUser());
			if(secure && !canSkipBouncer) {
				launchGallery(uri,adapter);
			} else{
				...
				intent.putExtra(KEY_SECURE_GALLERY, true);
	 ...
	 public Intent getGalleryIntent() {
	  ...
	  public void launchGallery(Uri uri, LocalDataAdapter adapter) {
        ...
        intent.putExtra(KEY_FROM_SNAPCAM, true);
		intent.putExtra(KEY_SECURE_GALLERY, true);
        intent.putExtra(KEY_TOTAL_NUMBER, CameraUtil.thumbCount);
        boolean wouldLaunchResolverActivity = wouldLaunchResolverActivity(
                this, intent, KeyguardUpdateMonitor.getCurrentUser());
	  ...
	  public  boolean wouldLaunchResolverActivity(Context ctx, Intent intent,
            int currentUserId) {
     ...
     */
    public  ActivityInfo getTargetActivityInfo(Context ctx, Intent intent,
	 ...
	 private static boolean wouldLaunchResolverActivity(
	  ...
	  private  boolean packageExist(Context context, String packageName) {
	  ...
	  private void exitSecurePhotoView(){
        camera_above_filmstrip_layout_secure.startAnimation(mHideAction);
        camera_above_filmstrip_layout_secure.setVisibility(View.GONE);
        mFilmStripView.startAnimation(mShowAction);
        mFilmStripView.setVisibility(View.VISIBLE);
    }
    ...
    private class CircularDrawable extends Drawable {
        ...
        public void draw(Canvas canvas) {
            canvas.drawRoundRect(new RectF(mRect), (mRect.right - mRect.left) / 10,
                    (mRect.bottom - mRect.top) / 10, mPaint);
        }
    ...
    public void onCreate(Bundle state) {
        ...
        if (!mSecureCamera) {
            ...
            /*for secure view photo pre*/
            mShowAction = new AnimationSet(true);
            mShowAction.addAnimation(new ScaleAnimation(2.0f, 1.0f, 2.0f, 1.0f, Animation.RELATIVE_TO_SELF, 0.5f, Animation.RELATIVE_TO_SELF, 0.5f));
            mShowAction.addAnimation(new AlphaAnimation(0.0f, 1.0f));
            mShowAction.setDuration(220);
            
            mHideAction = new AnimationSet(true);
            mHideAction.addAnimation(new ScaleAnimation(1.0f, 2.0f, 1.0f, 2.0f, Animation.RELATIVE_TO_SELF, 0.5f, Animation.RELATIVE_TO_SELF, 0.5f));
            mHideAction.addAnimation(new AlphaAnimation(1.0f, 0.0f));
            mHideAction.setDuration(220);

            camera_above_filmstrip_layout_secure = (FrameLayout) findViewById(R.id.camera_above_filmstrip_layout_secure);
            imgs_viewpager = (PhotoViewPager) findViewById(R.id.imgs_viewpager);
            imgs_viewpager.setPageMargin(50);
            imgs_viewpager.setOffscreenPageLimit(3);
        }
        ...
        registerReceiver(mBatteryLowReceiver, filter);
    }
	...
	public void startOrStopRecord(boolean isStart) {
		if(isStart) {
			stopExit();
		} else{
			startExit();
    ...
    public void onResume() {
        ...
        //as Android doesna€?t have any default intent to share the state.
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
        ...
        if(mSecureCamera && camera_above_filmstrip_layout_secure.getVisibility() == View.VISIBLE){
            exitSecurePhotoView();
            return true;
        }
    ...
    public void requestLocationPermission() {
        if (ENABLE_RECORD_LOCATION && checkSelfPermission(Manifest.permission.ACCESS_COARSE_LOCATION)
            != PackageManager.PERMISSION_GRANTED) {
    ...
    private void initLayoutTransition(){
        ...
        mLayoutTransition.setStartDelay(LayoutTransition.DISAPPEARING, 200);

        mCameraRootFrame.setLayoutTransition(mLayoutTransition);
    }

    private Animator getAppearingAnimation() {
        AnimatorSet mSet = new AnimatorSet();
        mSet.playTogether(ObjectAnimator.ofFloat(null, "ScaleX", 1.0f, 1.0f),
                ObjectAnimator.ofFloat(null, "ScaleY", 1.0f, 1.0f),
                ObjectAnimator.ofFloat(null, "Alpha", 1.0f, 1.0f));
        return mSet;
    }
    ...
    private void setModuleFromIndex(int moduleIndex) {
      ...
      if(!((moduleIndex == ModuleSwitcher.MAKEUP_MODULE_INDEX && mCurrentModuleIndex == ModuleSwitcher.PHOTO_MODULE_INDEX)
            ||(moduleIndex == ModuleSwitcher.PHOTO_MODULE_INDEX && mCurrentModuleIndex == ModuleSwitcher.MAKEUP_MODULE_INDEX)))
        ...
        switch (moduleIndex) {
            ...
            case ModuleSwitcher.MAKEUP_MODULE_INDEX:
               ...
               ((PhotoModule) mPhotoModule).switchMakeup();
                if(!(mCurrentModule instanceof PhotoModule))
	...
	public void getBlurData() {
		 mCurrentModule.setOneShotPreview();
	}
}
