...
import com.android.camera.ui.PopupListMenu;
...
import com.android.camera.widget.SwitcherBar;

public class PhotoMenu extends MenuController
        implements PopupListMenu.Listener,
    ...
    public void initialize(PreferenceGroup group) {
        ...
        initFlashModeButton(mFlashSwitcher);
        if(TsMakeupManager.HAS_TS_MAKEUP) {
        ...
        if(!TsMakeupManager.HAS_TS_MAKEUP) {
            ...
            if (group.findPreference(CameraSettings.KEY_CAMERA_HDR) != null) {
               ...
               // initSwitchItem(CameraSettings.KEY_CAMERA_HDR, mHdrSwitcher);
				initHdrItem(CameraSettings.KEY_CAMERA_HDR, mHdrSwitcher);
            } else {
                mHdrSwitcher.setVisibility(View.INVISIBLE);
            }
        }
    ...
    public boolean handleBackKey() {
        ...
        if (mPopupStatus == POPUP_FIRST_LEVEL) {
            animateSlideOutY(mSettingPreview, 1);
        } else if (mPopupStatus == POPUP_SECOND_LEVEL) {
            animateSlideOutX(mListSubMenu, 2);
    ...
    public void closeSceneMode() {
		Log.d(TAG,"closeSceneMode mPreviewMenuLayout");
    ...
    public void animateSlideInX(final View v) {
        ViewPropertyAnimator vp = v.animate();
        v.setScaleX(0f);
        v.setPivotX(0f);
        vp.scaleX(1.0f);
	...
	public void animateSlideInY(final View v) {
        ViewPropertyAnimator vp = v.animate();
        v.setScaleY(0f);
        v.setPivotY(0f);
        vp.scaleY(1.0f);
    ...
    private void animateSlideOutX(final ListView v, final int level) {
        if (v == null || mPopupStatus == POPUP_IN_ANIMATION_SLIDE)
            return;
        mPopupStatus = POPUP_IN_ANIMATION_SLIDE;
        ...
        ViewPropertyAnimator vp = v.animate();
        v.setPivotX(0f);
        vp.scaleX(0).setDuration(ANIMATION_DURATION);
        vp.setListener(new AnimatorListener() {
            @Override
            public void onAnimationStart(Animator animation) {
            }
            ...
            public void onAnimationEnd(Animator animation) {
                if (level == 1) {
                    ...
                    mPopupStatus = POPUP_FIRST_LEVEL;
					//((ListMenu) mListMenu).resetHighlight();
					Log.d(TAG,"animateSlideOutX onAnimationEnd level2");
                }
            }
            ...
            public void onAnimationCancel(Animator animation) {
                if (level == 1) {
                    ...
                    mPopupStatus = POPUP_FIRST_LEVEL;
					//((ListMenu) mListMenu).resetHighlight();
					Log.d(TAG,"animateSlideOutX onAnimationCancel level2");
                }
	...
	private void animateSlideOutY(final View v, final int level) {
		   if (v == null || mPopupStatus == POPUP_IN_ANIMATION_SLIDE)
			   return;
		   mPopupStatus = POPUP_IN_ANIMATION_SLIDE;
		   ...
		   ViewPropertyAnimator vp = v.animate();
		   v.setPivotY(0f);
		   vp.scaleY(0).setDuration(ANIMATION_DURATION);
		   vp.setListener(new AnimatorListener() {
			   @Override
			   public void onAnimationStart(Animator animation) {
			   }
    ...
    public void animateSlideOutPreviewMenu() {
        if(TsMakeupManager.HAS_TS_MAKEUP && mTsMakeupManager.isShowMakeup()) {
		 ...
		 Log.d(TAG,"animateSlideOutPreviewMenu mPreviewMenuStatus:"+mPreviewMenuStatus);
		 if (mPreviewMenuStatus == PREVIEW_MENU_ON) {
            animateSlideOut(mPreviewMenu);
        }
		 Log.d(TAG,"animateSlideOutPreviewMenu mPopupStatus:"+mPopupStatus);
        if (mPopupStatus == POPUP_NONE)
            return;
        if (mPopupStatus == POPUP_FIRST_LEVEL) {
            //animateSlideOutY(mListMenu, 1);
			animateSlideOutY(mSettingPreview,1);
        }  else if(mPopupStatus == POPUP_SECOND_LEVEL) {
			closeView();
		}
	...
	public void showOrHideFlashButton(int cameraID) {
		Log.d(TAG,"showOrHideFlashButton cameraID:"+cameraID);
		mCameraId = cameraID;
		boolean show = (mCameraId != 3);
		mFlashSwitcher.setVisibility(show ? View.VISIBLE : View.INVISIBLE);
	}
    ...
    protected void initializePopup() {
        ...
        PopupListMenu popup1 = (PopupListMenu) inflater.inflate(
                R.layout.popup_list_menu, null, false);
		...
		Log.d(TAG,"initializePopup bokeh:"+bokeh);
        if ("2".equals(bokeh)) {
           // buttonSetEnabled(mHdrSwitcher,false);
    ...
    public void initSwitchItem(final String prefKey, View switcher) {
        ...
        switcher.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
            	if(CameraUtil.isFastSwitcher()) {
					return;
				}
				Log.d(TAG,"isFastSwitcher is false");
                if (mPreviewMenuStatus == PREVIEW_MENU_ON) {
                    animateSlideOut(mPreviewMenu);
                    return;
                }
                ...
                if (pref == null)
                    return;
                int index = pref.findIndexOfValue(pref.getValue());
				Log.d(TAG,"initSwitchItem index0:"+index);
                CharSequence[] values = pref.getEntryValues();
				Log.d(TAG,"initSwitchItem length:"+values.length+",isBokeh:"+isBokeh);
				if(isBokeh) {
					if(index == 0) {
						index = 2;
					} else{
				...
				Log.d(TAG,"initSwitchItem index:"+index);
                reloadPreference(pref);
               	onSettingChanged(pref);
                enableFrontBackSwitcher(false);
            }
        });
    }

	
	public void initHdrItem(final String prefKey, View switcher) {
		   final IconListPreference pref =
				   (IconListPreference) mPreferenceGroup.findPreference(prefKey);
		   if (pref == null)
			   return;
		   Log.d(TAG,"initHdrItem isHdr:"+isHdr);
		   if(isHdr && mCameraId == CameraHolder.instance().getFrontCameraId()) {
			 	setPreference(CameraSettings.KEY_CAMERA_HDR,"on");
		  	}
		   ...
		   switcher.setOnClickListener(new OnClickListener() {
			   ...
			   public void onClick(View v) {
				  ...
				  if(index == 1 && isBokeh){
				  	 isHdr = true;
					 isBokeh = false;
					 mListener.onCameraPickerClicked(CameraHolder.instance().getFrontCameraId());
				  } else{
				   	pref.setValueIndex(index);
				   	((ImageView) v).setImageResource(
					  		 ((IconListPreference) pref).getLargeIconIds()[index]);
    ...
    public void initBokehModeButton(View button) {
		...
		if(mCameraId == CameraHolder.instance().getFrontCameraId()) {
			 ...
			 iv.setImageResource(iconIds[1]);
			 bokehPref.setValueIndex(1);
			 onSettingChanged(bokehPref);
		} 
       

        button.setVisibility(mCameraId !=CameraHolder.instance().getBackCameraId()?View.VISIBLE:View.GONE);
        ...
        button.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
				...
				if(mCameraId == CameraHolder.instance().getFrontCameraId()) {
					 ...
					 flashIndex = pref.findIndexOfValue(pref.getValue());
					 mListener.onCameraPickerClicked(2);
               ...
               if (bokeh != null) {
					...
					Log.d(TAG,"initBokehModeButton index:"+index);
                    ((ImageView) v).setImageResource(
                            ((IconListPreference) pref).getLargeIconIds()[index]);
                    reloadPreference(pref);
                    initializePopup();
    ...
    public void initFlashModeButton(View button) {
        // ((ImageView) button).setImageResource(R.drawable.ic_settings_flash);
        final IconListPreference pref = (IconListPreference) mPreferenceGroup.findPreference(CameraSettings.KEY_FLASH_MODE);
		Log.d(TAG,"initFlashModeButton pref:"+pref);
        if(pref == null){
            return;
        }
		if(flashIndex != -1) {
			pref.setValueIndex(flashIndex);
			onSettingChanged(pref);
		}
        updateFlashModeIcon(pref);
        button.setVisibility(View.VISIBLE);
        button.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
    ...
    public void addFlashLayout(){
        ...
        // if (portrait) {
            gridRes = R.layout.vertical_switcher_bar;
        // } else {
            // gridRes = R.layout.horiz_switcher_bar;
        // }
        mPreviewMenuStatus = PREVIEW_MENU_ON;
        previewMenuSize = mFlashSwitcher.getHeight();
        ...
        previewMenuLayout.setTranslationY(previewMenuSize - 70);
        basic.setLayoutParams(new FrameLayout.LayoutParams(LayoutParams.MATCH_PARENT,
                LayoutParams.MATCH_PARENT));
        final IconListPreference pref = (IconListPreference) mPreferenceGroup.findPreference(CameraSettings.KEY_FLASH_MODE);
        SwitcherBar layout = (SwitcherBar) basic.findViewById(R.id.layout);
        layout.setUnusable(false);
		Log.d(TAG,"addFlashLayout cameraID:"+mCameraId);
		if(mCameraId != CameraHolder.instance().getBackCameraId()) {
	       layout.initData(new String[]{mActivity.getString(R.string.pref_camera_flashmode_entry_off),
                                                    mActivity.getString(R.string.pref_camera_flashmode_entry_auto),
                                                    mActivity.getString(R.string.pref_camera_flashmode_entry_on)}, R.drawable.front_flash_off, pref.getCurrentIndex());
		} else{
			layout.initData(new String[]{mActivity.getString(R.string.pref_camera_flashmode_entry_off),
                                                    mActivity.getString(R.string.pref_camera_flashmode_entry_auto),
                                                    mActivity.getString(R.string.pref_camera_flashmode_entry_on)}, R.drawable.ic_settings_flash, pref.getCurrentIndex());
		}
        
        layout.setItemSelectionListener(new SwitcherBar.ItemSelectionListener(){
            @Override
                public void onSelection(int position, int id) {
    ...
    public int getPreviewMenuSize(){
        return previewMenuSize;
    }

    public void updateFlashModeIcon(IconListPreference pref) {
		if(pref == null) {
			return;
		}
		TypedArray mTypedArray;
		Log.d(TAG,"updateFlashModeIcon cameraID:"+mCameraId);
		boolean front = (mCameraId != CameraHolder.instance().getBackCameraId());
		if(front) {
			mTypedArray = mActivity.getResources().obtainTypedArray(R.array.front_camera_flashmode_icons);
		} else{
			mTypedArray = mActivity.getResources().obtainTypedArray(R.array.camera_flashmode_icons);
		}
        int ind = pref.getCurrentIndex();
        if (ind == -1)
            ind = 0;
        ((ImageView) mFlashSwitcher).setImageResource(mTypedArray.getResourceId(ind, R.drawable.ic_settings_flash));
        mTypedArray.recycle();
    }
    ...
    public void addSceneMode() {
        ...
        if (portrait) {
            gridRes = R.layout.vertical_grid;
             size = width;
            //size = mSceneModeSwitcher.getHeight();
        } else {
        ...
        previewMenuLayout.setTranslationY(size - 20);
        basic.setLayoutParams(new FrameLayout.LayoutParams(LayoutParams.MATCH_PARENT,
                LayoutParams.MATCH_PARENT));
        ViewGroup layout = (ViewGroup) basic.findViewById(R.id.layout);
    ...
    public void updateSceneModeIcon(IconListPreference pref) {
	  ...
	  ((ImageView) mFlashSwitcher).setImageResource(thumbnails[ind]);
    }
    ...
    public void addFilterMode() {
        ...
        if (portrait) {
            gridRes = R.layout.vertical_grid;
            // size = width;
            size = mFilterModeSwitcher.getHeight();
        } else {
            gridRes = R.layout.horiz_grid;
        }
        previewMenuSize = size;
        ...
        if (portrait) {
            ...
            previewMenuLayout.setLayoutParams(params);
            ((ViewGroup) mUI.getRootView()).addView(previewMenuLayout);
            previewMenuLayout.setTranslationY(size - 70);
        } else {
            params = new ViewGroup.LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.MATCH_PARENT);
            previewMenuLayout.setLayoutParams(params);
            ((ViewGroup) mUI.getRootView()).addView(previewMenuLayout);
        ...
        for (int i = 0; i < entries.length; i++) {
                ...
                public boolean onTouch(View v, MotionEvent event) {
                    if (event.getAction() == MotionEvent.ACTION_DOWN) {
                        ...
                        if (System.currentTimeMillis() - startTime < CLICK_THRESHOLD) {
                            ...
                            image.setBackgroundColor(mActivity.getColor(R.color.mode_selected_text_color));
                        }
                    }
                    return true;
                }
            ...
            views[j] = imageView;
            if (i == init)
                imageView.setBackgroundColor(mActivity.getColor(R.color.mode_selected_text_color));
            TextView label = (TextView) layout2.findViewById(R.id.label);
            imageView.setImageResource(thumbnails[i]);
            label.setText(entries[i]);
            layout.addView(layout2);
    ...
    public void openFirstLevel() {
        ...
        if(isPreviewMenuBeingShown() || CameraControls.isAnimating()){
            mUI.onBackPressed();
        }
        if(TsMakeupManager.HAS_TS_MAKEUP) {
            if(mTsMakeupManager.isShowMakeup()) {
    ...
    public void onPreferenceClicked(ListPreference pref, int y) {
        ...
        mListSubMenu = basic;
        mUI.removeLevel2();
		Log.d(TAG,"onPreferenceClicked mPopupStatus:"+mPopupStatus);
        if (mPopupStatus == POPUP_SECOND_LEVEL) {
    ...
    public void removeAllView() {
		...
		Log.d(TAG,"removeAllView mPreviewMenuLayout");
        closeSceneMode();
        mPreviewMenuStatus = PREVIEW_MENU_NONE;
    }
    ...
    public void closeView() {
        if (mUI != null){
            ...
            animateSlideOutX(mListSubMenu, 2);
           // ((ListMenu) mListMenu).resetHighlight();
			Log.d(TAG,"closeView mPopupStatus:"+mPopupStatus);
        }
    ...
    public void onSettingChanged(ListPreference pref) {
        ...
        // set to non-auto.
        Log.d(TAG,"onSettingChanged getKey:"+pref.getKey()+",getValue:"+pref.getValue());
        if (same(pref, CameraSettings.KEY_SCENE_MODE, Parameters.SCENE_MODE_HDR)) {
			...
			scenePref = mPreferenceGroup.findPreference(CameraSettings.KEY_BOKEH_MODE);
			((ImageView) mBokehSwitcher).setImageResource(
                            ((IconListPreference) scenePref).getLargeIconIds()[0]);
			 setPreference(CameraSettings.KEY_BOKEH_MODE,"0");
			// isHdr = true;
           // updateSceneModeIcon((IconListPreference) scenePref);
            ...
            if (longshotPref != null ) {
				...
				Log.d(TAG,"KEY_LONGSHOT KEY_ADVANCED_FEATURES:");
        ...
        if (same(pref, CameraSettings.KEY_BOKEH_MODE, "2")) {
    ...
    public void enableFlashSwitcher(boolean enable){
        IconListPreference pref = (IconListPreference) mPreferenceGroup.findPreference(CameraSettings.KEY_FLASH_MODE);
        updateFlashModeIcon(pref);
        mFlashSwitcher.setEnabled(enable);
    }

     public void enableHdrSwitcher(boolean enable){
        mHdrSwitcher.setEnabled(enable);
    }

	public void setSettingPreview(View preview) {
		this.mSettingPreview = preview;
	}
    public void enableFrontBackSwitcher(boolean enable){
