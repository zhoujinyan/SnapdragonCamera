...
public class ZLayout extends ViewGroup {
	...
	public ZLayout(Context context, AttributeSet attrs) {
		...
		// R.styleable.ZLayout, 0, 0);
		try {
			mLineSpacing = 0;// ta.getDimension(R.styleable.ZLayout_lineSpacing,
								// 0);
			mMaxLines = Integer.MAX_VALUE;// ta.getInt(R.styleable.ZLayout_maxLines,
											// Integer.MAX_VALUE);
	...
	private int measureWidth(int widthMeasureSpec) {
		...
		Log.d(TAG,
				"enter measureWidth(), widthMeasureSpec: "
		...
		Log.d(TAG, "after measureWidth(), mWidth: " + width + ", mLineCount: "
				+ mLineCount);
	...
	private int measureHeight(int heightMeasureSpec) {
		...
		Log.d(TAG,
				"enter measureHeight(), heightMeasureSpec: "
		...
		for (int i : mBreakAtChild) {
			mMaxHeightInEachLine
					.add(getMeasuredHeightWithMargins(getChildAt(i)));
		}
		...
		int totalSpacing = mLineCount > 1 ? (int) ((mLineCount - 1) * mLineSpacing)
				: 0;
		...
		case MeasureSpec.AT_MOST:
			height = Math.min(needHeight + totalSpacing + getPaddingBottom()
					+ getPaddingTop(), heightSize);

			break;

		case MeasureSpec.UNSPECIFIED:
			height = needHeight + totalSpacing + getPaddingBottom()
					+ getPaddingTop();
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		for (int i = 0, j = 0; i < mChildCountNeedLayout; i++) {
			...
			child.layout(startX + leftMargin, startY + topMargin, startX
					+ leftMargin + childWidth, startY + topMargin + childHeight);
